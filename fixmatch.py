...
def set_seed(seed=42):
    ...
    torch.cuda.manual_seed_all(seed) # if u r using multi-GPU
...
def load_data(args,cfg):
    ...
    label_count = len(os.listdir(os.path.join(args.labeled_data_dir, 'label3')))
    ...
    valset = KerogensDataset(
        ...
        args.val_data_dir, 'val',args,cfg
    ...
    trainloader_u =DataLoader(
        trainloader_u,batch_size=cfg['batch_size'],
        pin_memory=True,nnum_workers=1,drop_last=True
    )
    ...
    valloader =DataLoader(
        valset,batch_size=cfg['batch_size'],pin_memory=True,num_workers=1,drop_last=True
    )
...
def run_epoch(
    ...
    model,optimizer,criterion_l,criterion_u,
